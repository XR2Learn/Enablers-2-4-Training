...
class CNN1D(LightningModule):
    def __init__(self,
        ...
        self.conv_block1 = nn.Sequential(OrderedDict([
            ('conv1', nn.Conv1d(in_channels=in_channels, out_channels=out_channels[0], kernel_size=self.kernel_sizes[0],
        ...
        self.conv_block2 = nn.Sequential(OrderedDict([
            ('conv2',
             nn.Conv1d(in_channels=out_channels[0], out_channels=out_channels[1], kernel_size=self.kernel_sizes[1],
                       stride=stride, padding=padding)),
        ...
        self.conv_block3 = nn.Sequential(OrderedDict([
            ('conv3',
             nn.Conv1d(in_channels=out_channels[1], out_channels=out_channels[2], kernel_size=self.kernel_sizes[2],
                       stride=stride, padding=padding)),
        ...
        self.out_size = self._compute_out_size(len_seq, padding, self.kernel_sizes, stride, 3, out_channels[-1],
                                               pool_size, pool_padding)
    ...
    @staticmethod
    def _compute_out_size(sample_length, padding, kernel_sizes, stride, num_layers, num_channels, pool_size,
                          pool_padding):
...
class CNN1D1L(LightningModule):
    def __init__(self,
        ...
        Args:
            ...
            kernel_sizes: the size of the convolutional kernel
            strid: size of the stride
            padding: unused, just to compute the out size
            pool_size: size of the maxpooling 
            p_drop: dropout value
        ...
        self.out_size = self._compute_out_size(len_seq, padding, self.kernel_sizes, stride, self.num_layers,
                                               out_channels[-1], pool_size, pool_padding)
