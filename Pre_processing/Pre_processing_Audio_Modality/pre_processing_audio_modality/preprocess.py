...
def preprocess():
    """
    Preprocesses the wav dataset defined inside of the configuration using the given parameters.
    ...
    for phase in ['train','val','test']:
        ...
        for subject_path in tqdm(subjects,desc=f"preprocessing {phase} set"):
            ...
            for audio_path in all_subject_audio_files:
                ...
                assert ((sr/CUSTOM_SETTINGS['pre_processing_config']['target_sr']) - 0.1) < (len(audio)/len(resampled_audio)) < ((sr/CUSTOM_SETTINGS['pre_processing_config']['target_sr']) + 0.1), f"{audio_path} was not resampled correctly"
                
                all_subject_audio.append(resampled_audio)
                loaded_files.append(audio_path)

            all_subject_audio_processed = preprocessing_to_aply(all_subject_audio)
            ...
            if CUSTOM_SETTINGS['pre_processing_config']['padding']:
                for i,processed_audio in enumerate(all_subject_audio_processed):
                    if len(processed_audio)>(CUSTOM_SETTINGS['pre_processing_config']['target_sr']*CUSTOM_SETTINGS['pre_processing_config']['max_length']):
                        processed_audio = processed_audio[:int(CUSTOM_SETTINGS['pre_processing_config']['target_sr']*CUSTOM_SETTINGS['pre_processing_config']['max_length'])]
                    elif len(processed_audio)<(CUSTOM_SETTINGS['pre_processing_config']['target_sr']*CUSTOM_SETTINGS['pre_processing_config']['max_length']):
